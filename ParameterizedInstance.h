...
public:
    ...
    ParameterizedInstance(WorkingCopyInstance& inst, WorkingCopyGraph::NodeMap<WorkingCopyGraph::Node>& aref, WorkingCopyGraph::NodeMap<WorkingCopyGraph::Node>& bref, ParameterizedInstance& paramInst) : _instance(inst), _parameter(numeric_limits<double>::signaling_NaN()), _lbValue(0.0), _lbDecrease(inst.getGraph(), 0.0), _conflictTriple(inst.getGraph()), _nodesLowerBound(inst.getGraph(), 0.0), _edgesLowerBound(inst.getGraph(), 0.0), _icfCrossRef(inst.getGraph()), _icf(inst.getGraph()), _icpCrossRef(inst.getGraph()), _icp(inst.getGraph()) {
        for(WorkingCopyGraph::NodeIt u(_instance.getGraph()); u != INVALID; ++u) {
            _icpCrossRef[u] = new WorkingCopyGraph::NodeMap<int>(_instance.getGraph(), -1);
            _icp[u] = new MaxHeap(*_icpCrossRef[u]);
...
private:
    ...
    void updateLowerBoundBeforeSetForbidden(const WorkingCopyGraph::Edge& uv, vector<WorkingCopyGraph::Node*>& cts);
    void updateLowerBoundBeforeSetPermanent(const WorkingCopyGraph::Edge& uv, const WorkingCopyGraph::Node& proxy);
    void computeLowerBound(const WorkingCopyGraph::Edge &uv, BinHeap<double, WorkingCopyGraph::EdgeMap<int>, std::greater<double> > &edgesOrderByWeight, WorkingCopyGraph::EdgeMap<double>& weight, WorkingCopyGraph::EdgeMap<bool>& candidate, vector<WorkingCopyGraph::Node*>& cts);
    ...
    vector<WorkingCopyGraph::Node* > _conflictTriples;
